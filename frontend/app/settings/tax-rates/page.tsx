...
export default function TaxRatesPage() {
  ...
  const fetchTaxRates = async () => {
    ...
    try {
      ...
      const { data } = await api.get('/api/v1/tax-rates', { params });
  ...
  useEffect(() => { fetchTaxRates(); }, [stateFilter]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!payload.state_code) payload.state_code = null;
      if (!payload.expiry_date) payload.expiry_date = null;
      if (!payload.notes) payload.notes = null;

      await api.post('/api/v1/tax-rates', payload);
      setShowForm(false);
      ...
      setError(err.response?.data?.detail || 'Failed to create tax rate');
    }
  };
